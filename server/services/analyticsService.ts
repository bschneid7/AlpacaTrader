...
class AnalyticsService {
   ...
   */
  async getMonthlyReturns(userId: string): Promise<Array<{ month: string; return: number }>> {
    try {
      ...
      const returns: Array<{ month: string; return: number }> = [];
  ...
  async getPerformanceMetrics(userId: string): Promise<{
    ...
    bestTrade: { symbol: string; pl: number; plPercent: number };
    worstTrade: { symbol: string; pl: number; plPercent: number };
    avgWin: number;
    avgLoss: number;
    sharpeRatio: number;
    maxDrawdown: number;
   ...
   */
  async getTradeHistory(userId: string): Promise<
    Array<{
      date: string;
      symbol: string;
      entryPrice: number;
   ...
   */
  private calculateSharpeRatio(portfolioHistory: Array<{ portfolioValue: number; date: Date }>): number {
    try {
   ...
   */
  private calculateMaxDrawdown(portfolioHistory: Array<{ portfolioValue: number; date: Date }>): number {
    try {
   ...
   */
  async recordPortfolioSnapshot(userId: string, portfolioData: {
