...
router.get('/portfolio-history', requireUser, async (req, res) => {
    try {
        const userId = req.user._id.toString();
        const { timeframe = '1M' } = req.query;
        console.log(`GET /api/analytics/portfolio-history - User: ${userId}, Timeframe: ${timeframe}`);
        const data = await analyticsService.getPortfolioHistory(userId, timeframe);
...
router.get('/monthly-returns', requireUser, async (req, res) => {
    try {
        const userId = req.user._id.toString();
...
router.get('/performance-metrics', requireUser, async (req, res) => {
    try {
        const userId = req.user._id.toString();
...
router.get('/trade-history', requireUser, async (req, res) => {
    try {
        const userId = req.user._id.toString();
...
router.post('/portfolio-snapshot', requireUser, async (req, res) => {
    try {
        const userId = req.user._id.toString();
