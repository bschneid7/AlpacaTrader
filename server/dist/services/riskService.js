...
class RiskService {
     ...
     */
    async updateRiskLimits(userId, updates) {
        try {
            ...
            const riskLimits = await RiskLimits.findOneAndUpdate({ userId }, { $set: updates }, { new: true, upsert: true });
            if (!riskLimits) {
     ...
     */
    async emergencyStopAllPositions(userId) {
        try {
            ...
            // Close all positions
            const closePromises = alpacaPositions.map(async (position) => {
                try {
                    console.log(`[RiskService] Closing position: ${position.symbol}`);
                    await alpacaClient.delete(`/v2/positions/${position.symbol}`);
                    // Update position in database
                    await Position.findOneAndUpdate({ userId, symbol: position.symbol, status: 'open' }, {
                        status: 'closed',
            ...
            await TradingPreferences.findOneAndUpdate({ userId }, {
     ...
     * Calculate sector concentration
     */
    calculateSectorConcentration(positions, portfolioValue) {
     ...
     * Calculate position concentration
     */
    calculatePositionConcentration(positions, portfolioValue) {
        return positions
            .map(position => ({
     ...
     * Calculate volatility index (simplified)
     */
    calculateVolatilityIndex(positions, historicalMetrics) {
     ...
     */
    async checkRiskLimitBreaches(userId) {
        try {
